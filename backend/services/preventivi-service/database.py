...
if DATABASE_URL.startswith("postgresql://") and "+asyncpg" not in DATABASE_URL:
    ...
    print(f"[Preventivi] DATABASE_URL con +asyncpg")

# Aggiungi SSL per Render.com
...
# Connection pool size - ridotto per produzione
...
async def ensure_database_initialized():
    """Lazy initialization del database - connette solo al primo uso"""
    global _db_initialized
    if not _db_initialized:
        try:
            if not database.is_connected:
                await database.connect()
                print("[Preventivi] Database connesso (lazy init)")
            _db_initialized = True
        except Exception as e:
            print(f"[Preventivi] Errore connessione DB: {e}")
            raise
...
async def close_database():
    ...
    if database.is_connected:
        ...
        _db_initialized = False
        print("[Preventivi] Database disconnesso")
